...
// UDP autodiscovery settings
const unsigned int DISCOVERY_PORT = 31335;  // UDP port for autodiscovery
WiFiUDP udp;
unsigned long lastBroadcastTime = 0;
const unsigned long BROADCAST_INTERVAL = 3000;  // Broadcast every 3 seconds
...
void read_blocking_spi() {
    ...
    client.write(spi_report_message, command_buffer[1] + 5);
...
void setup() {
    ...
    // Start UDP for autodiscovery broadcast
    udp.begin(DISCOVERY_PORT);
    Serial.print("UDP autodiscovery started on port: ");
    Serial.println(DISCOVERY_PORT);
}

void loop() {
    // Broadcast IP address for autodiscovery
